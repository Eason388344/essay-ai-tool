...
# ====================== 修改计划（思维链） ==============================
def get_revision_plan(title, body, diagnosis_text, selected_options, custom_instruction, writing_style):
...
def revise_essay(title, original_body, diagnosis_text, selected_options, 
                 ...
                 sample_text="", target_paragraphs=None):
    """
    修缮引擎
    target_paragraphs: 列表，包含用户选中的段落原文。如果为空则执行全文修缮。
    """
    # 基础映射
    option_map = {
...
"""
    ...
    prompt = f"""【作文题目】：{title}
【原文】：
{marked_original if target_paragraphs and len(target_paragraphs) > 0 else original_body}
【诊断结果】：
{diagnosis_text}
{calibration_context}
{sample_context}
...
with st.sidebar:
    ...
    if st.button("📥 导出诊断报告", use_container_width=True):
        ...
        st.download_button("⬇️ 下载报告 (JSON)", report_json, "诊断报告.json", "application/json")
...
with tab1:
    ...
    with col2:
        st.markdown("**✏️ 手动编辑区**")
        genre = st.selectbox("文体", ["议论文", "记叙文", "说明文", "书信", "其他"], index=0)
        ...
        if body.strip():
            ...
            if is_valid:
                ...
                st.warning(f"⚠️ {msg}")
        if st.button("📌 确认文本并进入诊断", type="primary", use_container_width=True):
            is_valid, msg = validate_input(title, body)
            if is_valid:
...
with tab3:
    ...
    if not st.session_state.get("current_title"):
        ...
        with st.expander("📚 参考范文/素材上传（可选）"):
            ...
            sample_textarea = st.text_area("或直接粘贴范文内容", value=st.session_state.sample_text, height=150)
            if sample_textarea:
        ...
        with st.expander("💾 预设指令管理"):
            ...
            with col_p2:
                if st.session_state.get("preset_instructions"):
                    idx = st.selectbox("加载预设", range(len(st.session_state.preset_instructions)),
        ...
        with st.form("revise_form"):
            ...
            st.markdown("**③ 文风迁移**")
            style_option = st.selectbox("目标文风", ["标准 (高考一类文)", "批判犀利", "文学抒情", "逻辑严密"], index=0)
            style_map = {"标准 (高考一类文)": "标准", "批判犀利": "批判犀利", 
                        "文学抒情": "文学抒情", "逻辑严密": "逻辑严密"}
            ...
            if len(paragraphs) > 1:
                ...
                if selected_paras:
                    st.info(f"✅ 已选择 **{len(selected_paras)}** 段进行局部修缮")
                else:
                    st.info("📝 未选择任何段落，将执行 **全文修缮**")
            else:
            ...
            st.markdown("**⑤ 高级选项**")
            show_plan = st.checkbox("📋 先显示修改计划，确认后再执行", value=False)
                        ...
                        with st.spinner("正在评估修缮效果..."):
                            try:
                                ...
                                comparison = {
                                    ...
                                    "local_paragraphs": len(target_paras)  # 记录修改段落数
                                }
...
# ===== 底部 =====
st.sidebar.markdown("---")
st.sidebar.caption("📌 研究平台：信度校验 · 人机协同 · 段落级修缮 · 闭环评价")
